...
public abstract class AbstractConcurrentCache<K,V> extends ConcurrentHashMap<K, V> implements IHighCacheMap<K,V> {
	
	public final int ITEM_PUT_OP = 1 << 0;
	public final int ITEM_RETRIVE_OP = 1 << 1 ;
	public final int ITEM_REMOVE_OP = 1 <<2 ;
	
	private MCacheManager<K, V> cacheManager = null ;
	private IExpireKeyHandler<K,V> iExpireKeyAdaptor = null;
	...
	//所有的缓存公用一个并行队列执行器
	public final static IParallelQueueExecutor DEFAULT_CACHE_QUEUE_EXECUTOR =
			new DisruptorParallelQueueExecutor(Runtime.getRuntime().availableProcessors() * 2, 2 << 12);
//			ParallelQueueExecutorBuilder.getInstance().builderSuperFastParallelQueueExecutor(4, "cache-event-loop-default");
	...
	public AbstractConcurrentCache(String cacheTopic,IParallelQueueExecutor iParallelQueueExecutor,IExpireKeyHandler<K,V> iExpireKeyAdaptor,int initialCapacity, float loadFactor) {
		...
		this.cacheManager = new MCacheManager<K,V>(this, iParallelQueueExecutor, true);
	}

	public AbstractConcurrentCache(String cacheTopic,IParallelQueueExecutor iParallelQueueExecutor,IExpireKeyHandler<K,V> iExpireKeyAdaptor,int initialCapacity) {
		this(cacheTopic,iParallelQueueExecutor,iExpireKeyAdaptor,initialCapacity, DEFAULT_LOAD_FACTOR);
	}

	/**
	 * Constructs a new, empty map with a default initial capacity and load factor.
	 */
	public AbstractConcurrentCache(String cacheTopic,IParallelQueueExecutor iParallelQueueExecutor,IExpireKeyHandler<K,V> iExpireKeyAdaptor) {
		this(cacheTopic,iParallelQueueExecutor,iExpireKeyAdaptor,DEFAULT_INITIAL_CAPACITY, DEFAULT_LOAD_FACTOR);
	}

	public AbstractConcurrentCache(String cacheTopic,IExpireKeyHandler<K,V> iExpireKeyAdaptor) {
		this(cacheTopic,DEFAULT_CACHE_QUEUE_EXECUTOR,iExpireKeyAdaptor,DEFAULT_INITIAL_CAPACITY, DEFAULT_LOAD_FACTOR);
	}
	
	public AbstractConcurrentCache(String cacheTopic,IExpireKeyHandler<K,V> iExpireKeyAdaptor,int initialCapacity) {
		this(cacheTopic,DEFAULT_CACHE_QUEUE_EXECUTOR,iExpireKeyAdaptor,initialCapacity, DEFAULT_LOAD_FACTOR);
	}
	...
	public void clear() {
		...
		cacheManager.notifyListeners(new ObjectEvent<>( MCacheManager.clearCacheEvent));
	}
	...
	public V getWithMissCacheHandler(K key) throws NoMissCacheHandlerException {
		...
		if (e == null) {// 一级缓存取，如果一缓存没有，则到二级缓存取
			if(missCacheHandler == null){
				throw new NoMissCacheHandlerException("the key["+key+"] does not IMissCacheHandler to operator,please implemate the interface for IMissCacheHandler.");
			}
	...
	public class UpdateCacheRateEvent extends ObjectEvent<Object>{
		...
		public UpdateCacheRateEvent() {
			super();
		}
		public UpdateCacheRateEvent(boolean isBroken) {
			super(isBroken);
		}
		public UpdateCacheRateEvent(int eventType) {
			super(eventType);
		}
	...
	public V put(K key, V value) {
		if(value instanceof Collection){
			throw new UnsupportedOperationException("the cache in memory can not in collection ");
		}
		/** OpenSymphony END */
		if (value == null) {
	...
	private void notifyCacheRateUpdate(final K key,final int operateType){
		...
		UpdateCacheRateEvent updateCacheRateEvent  = new UpdateCacheRateEvent(MCacheManager.cacheRateUpdateEvent);
		...
		if(isStartupCheckCapacity.compareAndSet(false, true)){
			...
			cacheManager.notifyListeners(new ObjectEvent<>(MCacheManager.checkCapacityEvent));
		}
	}
