 ...
 * 并行队列 执行器
 * 这里弥补了ParallelQueueExecutor 的缺点：可以让同一种类型的车子在指定车道上跑。（也即同一种类型的 action 会被指定的 thread 执行）。消除频繁的上下问切换。
 * 因此叫 FastParallelQueueExecutor。因此是吞吐量优先，所以相对于 SuperFastParallelQueueExecutor 来叫还是慢了半拍的
 * 
...
public class FastParallelActionExecutor extends AbstractParallelActionExecutor{
	...
	private ThreadPoolExecutorIndexAllocator indexAllocator ;
	...
	public FastParallelActionExecutor(int threads,String prefix) {
		...
		for(int i=0;i<threads;i++){
			//半个小时之后还没有任务过来，则销毁线程
			pools[i] = new ThreadPoolExecutor(0, 1,30L, TimeUnit.MINUTES,new LinkedBlockingQueue<Runnable>(),new DefaultThreadFactory(prefix));
		}
		
		if(isPowerOfTwo(threads)){
			indexAllocator = new PowerOfTwoExecutorIndexChooser(threads);
		}else{
			indexAllocator = new GenericExecutorIndexChooser(threads);
		}
	}
	...
	public  void enParallelAction(String queueTopic,Action action){
		...
		if(actionQueue == null){
			try {
				...
				if (actionQueue == null) {
					actionQueue = new FastParallelActionQueue(pools[indexAllocator.allocator()]);
	...
	public void executeOneTimeAction(Action action) {
		IActionQueue<Action> fastActionQueue = 
				new FastParallelActionQueue(pools[indexAllocator.allocator()]);
