...
public class DisruptorParallelQueueExecutor extends AbstractParallelQueueExecutor{
	...
	private Disruptor<RunnableEvent> builderDisruptor(int ringBufferSize){
		Disruptor<RunnableEvent> disruptor = 
				new Disruptor<RunnableEvent>(new StringEventFactory(),ringBufferSize, DaemonThreadFactory.INSTANCE,ProducerType.SINGLE,new LiteBlockingWaitStrategy());
		disruptor.handleEventsWith(new RunnableEventHandler());
		disruptor.start();
		return disruptor;
	}
	
	public static class RunnableEventTranslator implements EventTranslatorOneArg<RunnableEvent, Runnable>{
	...
	public static class RunnableEventHandler implements EventHandler<RunnableEvent>{
		...
		public void onEvent(RunnableEvent arg0, long arg1, boolean arg2) throws Exception {
	...
	public void cronTrriger(boolean isAuto) {
		...
		if(isOperating.get()){
			...
			secondsInterval = size > 100 ?  5 : 10 ;
		}
		
		try {
			for(String topic:queueExecutorMapping.keySet()){
				if(topicLastExecuteTime.get(topic) >= TimeUnit.SECONDS.toMillis(secondsInterval)){
					queueExecutorMapping.remove(topic);
	...
	public void executeOneTime(Runnable command) {
		Disruptor<RunnableEvent> ringBuffer = disruptors[ringBufferExecutorIndexChooser.allocator()];
		runing(ringBuffer, command);
	}
}
