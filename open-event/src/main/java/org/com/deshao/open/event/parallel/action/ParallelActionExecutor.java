...
import org.com.deshao.open.event.parallel.DefaultThreadFactory;
import org.com.deshao.open.event.parallel.ThreadPoolContainer;
 ...
 * 【注意】适合无状态的Action instance
 * 因为可以动态扩展线程池数量，因此在突发流量的情况下可以适当的增大线程数量，提供并发处理的能力。
 * @author pengbingting
 */
public class ParallelActionExecutor extends AbstractParallelActionExecutor{
	...
	public ParallelActionExecutor(int corePoolSize, int maxPoolSize, int keepAliveTime,String prefix){
		
		this(corePoolSize, maxPoolSize, keepAliveTime, TimeUnit.MINUTES, prefix, new DiscardOldestPolicy());
	}
	
	public ParallelActionExecutor(int corePoolSize, int maxPoolSize, int keepAliveTime,TimeUnit unit, String prefix){
		
		this(corePoolSize, maxPoolSize, keepAliveTime, unit, prefix, new DiscardOldestPolicy());
	}
	 ...
	 */
	public ParallelActionExecutor(int corePoolSize, int maxPoolSize, int keepAliveTime, String prefix,RejectedExecutionHandler rejectedExecutionHandler) {
		
		this(corePoolSize, maxPoolSize, keepAliveTime, TimeUnit.MINUTES, prefix, rejectedExecutionHandler);
	}
	
	public ParallelActionExecutor(int corePoolSize, int maxPoolSize, int keepAliveTime,TimeUnit unit, String prefix,RejectedExecutionHandler rejectedExecutionHandler) {
		LinkedBlockingQueue<Runnable> workQueue = new LinkedBlockingQueue<Runnable>(Integer.MAX_VALUE);//无界队列，不需 处理策略
		if (prefix == null) {
			prefix = "";
		}
		this.prefix = prefix;
		ThreadFactory threadFactory = new DefaultThreadFactory(prefix);
		pool = new ThreadPoolExecutor(corePoolSize, maxPoolSize, keepAliveTime, unit, workQueue, threadFactory,rejectedExecutionHandler);
	}
	...
	public void cronTrriger(boolean isAuto) {
		...
		//1、
		final int size = paralleleQueue.size();
		if(size <= 0){
			return ;
		}
		...
		if(isOperating.get()){
			...
			//2、
			secondsInterval = size > 100 ? 5 : 10 ;//根据队列大小,动态remove last interval action queue
		}
		...
		try {
			for(Entry<String, IActionQueue<Action>> entry : paralleleQueue.entrySet()){
				...
				//队列中的 action 都执行完了
				if(actionQueue.getQueue().isEmpty() &&(System.currentTimeMillis() - actionQueue.getLastActiveTime() >= TimeUnit.SECONDS.toMillis(secondsInterval))){
